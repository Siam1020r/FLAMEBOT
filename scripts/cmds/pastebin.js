...
module.exports = {
  ...
  onStart: async function ({ api, event, args, messageReply, type }) {
    const permission = ["100044530383890","100018943662619"];
    if (!permission.includes(event.senderID)) {
      api.sendMessage(
        "You don't have enough permission to use this command. Only SIAM can do it.",
        event.threadID,
        event.messageID
      );
      return;
    ...
    if (!text && name) {
      ...
      const client = new PasteClient("py2uNSwHmSBHrlswFQmSomj4w7_-n5wV");
